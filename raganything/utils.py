...
def read_file_with_encoding_fallback(
    ...
    encodings: tuple[str, ...] = ("utf-8", "gbk", "latin-1", "cp1252"),
) -> str:
    ...
    Raises:
        RuntimeError: If none of the encodings can decode the file.
    """
    ...
    for enc in encodings:
        try:
            ...
            continue
    raise RuntimeError(
        f"Could not decode {file_path} with any supported encoding: {last_error}"
    )
...
async def insert_text_content(
    ...
    """Insert pure text content into LightRAG (convenience wrapper).

    Delegates to :func:`insert_text_content_with_multimodal_content` with
...
async def insert_text_content_with_multimodal_content(
    ...
    """
    Insert text content into LightRAG, optionally with multimodal content.
    ...
    # Build kwargs, only including multimodal args when provided
    ...
    if scheme_name is not None:
        kwargs["scheme_name"] = scheme_name

    await lightrag.ainsert(**kwargs)
